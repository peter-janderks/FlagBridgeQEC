...
from flagbridgeqec.sim.optimized_circuits.run_circuit import FT_protocol, check_hld
...
class Run_Decoder(object):
    def __init__(self, argv):
        ...
        for i in range(len(argv)-14):
            ...
            self.result_files.append(str(self.cir_id) + '/' + str(self.total_trials) + str(self.nm) + '.txt')
        ...
        for i in range(self.n_point):
            ...
            qeccode = FT_protocol(per, self.cir_id, self.ridle)
            ...
            for j in range(len(argv)-14):
                ...
                write_data_to_file(self.result_files[j],per, self.ler[j,i],conf_interval)
        
        print(self.ler,'ler')
        self.create_plot(self.ler[0])

    def prep_dataset(self,name):
        dataset_file = 'pI_1.0/' + str(name) + '.txt'
        if name == 'lld_':
...
def write_data_to_file(file,per,ler_m,conf_int_a):
    of = open(file, 'a')
    of.write(str(per))
    of.write('\t')
    of.write(str(ler_m))
...
def run_decoder(name):
    data_file= str(name) + 'cir' + str(cir_id)+ '_' + str(ridle) + str(nm) + '.txt'
...
#    total_runs = trials*cpus
    ...
    for i in range(len(errs)):
        per = errs[i]
        qeccode = FT_protocol(per, per/float(rp2), per/float(rpm), cir_id,
                              idling, ridle)
        # run the simulation
